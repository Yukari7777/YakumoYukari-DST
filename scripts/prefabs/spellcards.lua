...
local function curse(inst)
	...
	inst.components.spellcard:SetSpellFn(function(inst, owner)
		...
		if owner.yukari_classified ~= nil then
			owner.yukari_classified.inspellcurse:set(true)
		end
	end)
	inst.components.spellcard:SetTaskFn(function(inst, owner)
		...
		owner.components.combat:SetAttackPeriod(TUNING.WILSON_ATTACK_PERIOD * 0.75)
...
local function lament(inst) -- TODO : Recode with StartThread()
	...
	inst.components.spellcard:SetSpellFn(function(inst, owner)
		if inst.Activated or owner.yukari_classified ~= nil and owner.yukari_classified.inspellcommon:value() then
			return false
		end
		...
		local function DoSpawn()
			...
			owner:DoTaskInTime(SpawnDelay, function()
				...
				if LeftSpawnCount > 1 then 
					...
					inst.Activated = false
					if owner.yukari_classified ~= nil then
						owner.yukari_classified.inspellcommon:set(false)
					end
				end
			end)
		end
		...
		inst.Activated = true
		if owner.yukari_classified ~= nil then
			owner.yukari_classified.inspellcommon:set(true)
		end
		local x,y,z = owner.Transform:GetWorldPosition()
		local ents = TheSim:FindEntities(x, y, z, 100)
		if owner.components.playercontroller ~= nil then
